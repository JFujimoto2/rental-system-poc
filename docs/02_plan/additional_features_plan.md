...
**難易度:** 中　**新規テーブル:** 0　**工数目安:** 中
**詳細:** `docs/06_features/jobs/`

#### 概要
Solid Queue による定期実行タスク。8ジョブを実装。
滞納自動検出、契約/ML期限切れ、部屋ステータス同期、入金/支払予定の月次生成、
契約更新リマインド、保険期限切れアラート。

---

### Step 15: 業者マスタ + 工事管理 — 実装済

**難易度:** 中　**新規テーブル:** 2（Vendor, Construction）　**工数目安:** 中
**詳細:** `docs/06_features/14_vendor_construction.md`

#### 概要
業者マスタ（Vendor）と工事（Construction）の CRUD。
原状回復・修繕等の発注〜施工〜完了〜請求の管理。承認ワークフロー連携。

---

### Step 16: 契約更新管理 — 実装済

**難易度:** 中　**新規テーブル:** 1（ContractRenewal）　**工数目安:** 中
**詳細:** `docs/06_features/15_contract_renewal.md`

#### 概要
契約期限前の更新管理。通知→交渉→合意→新契約作成→旧契約 terminated のフロー。
ContractRenewalReminderJob で3ヶ月前に自動リマインド。

---

### Step 17: 問い合わせ・修繕依頼 — 実装済

**難易度:** 中　**新規テーブル:** 1（Inquiry）　**工数目安:** 中
**詳細:** `docs/06_features/16_inquiry.md`

#### 概要
入居者からの問い合わせ管理。修繕依頼・クレーム・質問等のカテゴリ管理。
優先度・担当者アサイン・工事連携。
...
**難易度:** 低　**新規テーブル:** 2（Key, KeyHistory）　**工数目安:** 小
**詳細:** `docs/06_features/17_key_management.md`

#### 概要
部屋の鍵と貸出/返却履歴の管理。入居時の貸出、退去時の回収、紛失記録。

---

### Step 19: 保険管理 — 実装済

**難易度:** 低　**新規テーブル:** 1（Insurance）　**工数目安:** 小
**詳細:** `docs/06_features/18_insurance.md`

#### 概要
建物・部屋単位の保険管理。火災保険、地震保険、借家人賠償等。
InsuranceExpirationJob で期限30日前に自動アラート。
